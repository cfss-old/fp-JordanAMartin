...
nopred <- nopred %>%
  ...
  # rename Var to NLTH, drop other useless columns
  mutate(Hand_Prediction = "No Look to Hand")%>%
  mutate(Hand_Latency = NA)%>%
  select(-FixationIndex, -Var, -Val)
...
hand_df_Fam <- hand_df[!grepl("Test", hand_df$Trial),]
hand_df_Fam <- hand_df_Fam %>%
  separate(Trial, into = c("Type", "Num"), sep = " ")
hand_df_Fam <- unite(hand_df_Fam, Trial, Type, Num, sep = "_")

hand_df_TT <- hand_df[!grepl("Fam", hand_df$Trial),]
hand_df_TT <- hand_df_TT %>%
  separate(Trial, into = c("Type1", "Type2", "Num"), sep = " ")
hand_df_TT <- unite(hand_df_TT, Trial, Type1, Type2, sep = " ")
hand_df_TT <- unite(hand_df_TT, Trial, Trial, Num, sep = "_")

# Join them back together
hand_df <- rbind(hand_df_TT,hand_df_Fam)
```
...
nopred <- nopred %>%
  filter(!grepl("Fam", nopred$Trial))%>%
  # rename Var to NLTB, drop other useless columns
  mutate(Body_Prediction = "No Look to body")%>%
  mutate(Body_Latency = NA)%>%
  select(-FixationIndex, -Var, -Val)
...
# Factor and add _ before numbers
body_df_Fam <- body_df[!grepl("Test", body_df$Trial),]
body_df_Fam <- body_df_Fam %>%
  separate(Trial, into = c("Type", "Num"), sep = " ")
body_df_Fam <- unite(body_df_Fam, Trial, Type, Num, sep = "_")

body_df_TT <- body_df[!grepl("Fam", body_df$Trial),]
body_df_TT <- body_df_TT %>%
  separate(Trial, into = c("Type1", "Type2", "Num"), sep = " ")
body_df_TT <- unite(body_df_TT, Trial, Type1, Type2, sep = " ")
body_df_TT <- unite(body_df_TT, Trial, Trial, Num, sep = "_")

# Join them back together
body_df <- rbind(body_df_TT,body_df_Fam)
```
...
FixDur_Fam <- FixDur_Fam %>%
  separate(Trial, into = c("Type", "Num"), sep = " ")
FixDur_Fam <- unite(FixDur_Fam, Trial, Type, Num, sep = "_")
```


```{r Final Data Frame, echo=TRUE, message=FALSE, warning=FALSE}
# Join Hand and Body
Prediction_Latency <- left_join(hand_df, body_df)
...
# ***********************************
# STUCK HERE, CAN'T GET THEM TO JOIN PROPERLY, WHYYYYYYYYYY
# Join to Attention
# untidyMOAP1 <- full_join(untidyMOAP, FixDur_Fam, by = c("ID", "Trial"))

# ***********************************


# untidyMOAP1 <- arrange(untidyMOAP1, Test, ID, Trial)

# Transform AOI fix times to ms
# untidyMOAP1 <- untidyMOAP1 %>%
#  transform(Fix_Screen = Time_Attending * 1000) %>%
  # Calculate Sample Rate
#  transform(Screen_Att = Fix_Screen / TrialLength_ms)
# # Separate Trial Numbers
# MOAP <- MOAP %>%
#   separate(Trial, into = c("Trial_Type", "Trial_Num"), sep = "_")
# 
